...
class Board {
    ...
    Board() {
        ...
        _numberOfJumps = new Stack<>();
        _totalOpen = (7 * 7) - _numBlocks;

        for (int i = 0; i < _board.length; i++) {
            if (i <= EXTENDED_SIDE * 2 - 1 || i >= EXTENDED_SIDE * 9) {
                _board[i] = BLOCKED;
                _numBlocks++;
            } else if (i == (13 + 10 + 1) || i == (100 - 4)) {
                _board[i] = BLUE;
                incrPieces(BLUE, 1);
            } else if (i == (100 - 10) || i == (10 * 3)) {
                _board[i] = RED;
                incrPieces(RED, 1);
            } else if (i % EXTENDED_SIDE == 0 || i % EXTENDED_SIDE == 1
                    || i % EXTENDED_SIDE == 9 || i % EXTENDED_SIDE == 10) {
                _board[i] = BLOCKED;
                _numBlocks++;
            } else {
                _board[i] = EMPTY;
    ...
    void clear() {
        ...
        for (int i = 0; i < _board.length; i++) {
            if (i <= EXTENDED_SIDE * 2 - 1 || i >= EXTENDED_SIDE * 9) {
                _board[i] = BLOCKED;
                _numBlocks++;
            } else if (i == (6 * 4) || i == (100 - 4)) {
                _board[i] = BLUE;
            } else if (i == (100 - 10) || i == (3 * 10)) {
                _board[i] = RED;
            } else if (i % EXTENDED_SIDE == 0
                    || i % EXTENDED_SIDE == 1 || i % EXTENDED_SIDE == 9
                    || i % EXTENDED_SIDE == 10) {
                _board[i] = BLOCKED;
                _numBlocks++;
            } else {
    ...
    void jumpHelper(Move move) {
        set(move.fromIndex(), EMPTY);
        set(move.toIndex(), whoseMove());
        PieceColor opponent = _whoseMove.opposite();
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (_board[neighbor(move.toIndex(), i, j)] == opponent) {
                    set(neighbor(move.toIndex(), i, j), whoseMove());
        ...
        _numMoves++;
        _numberOfJumps.push(_numJumps);
        _numJumps++;
    }

    void extendHelper(Move move) {
        set(move.toIndex(), whoseMove());
        PieceColor opponent = _whoseMove.opposite();
        incrPieces(whoseMove(), 1);
        for (int i = -1; i <= 1; i++) {
    ...
    boolean canMove(PieceColor who) {
        for (int i = 0; i < _board.length; i++) {
            if (_board[i] == who) {
                if (_board[i + 11] == EMPTY || _board[i - 11] == EMPTY
                        || _board[i + 12] == EMPTY
                        || _board[i - 12] == EMPTY || _board[i - 1] == EMPTY
                        || _board[i + 1] == EMPTY
                        || _board[i + 10] == EMPTY || _board[i - 10] == EMPTY
                        || _board[i + (10 + 10)] == EMPTY
                        || _board[i - (10 * 2)] == EMPTY
                        || _board[i + (10 + EXTENDED_SIDE)] == EMPTY
                        || _board[i + (EXTENDED_SIDE + EXTENDED_SIDE)] == EMPTY
                        || _board[i + (EXTENDED_SIDE + 2)] == EMPTY
                        || _board[i + (EXTENDED_SIDE + 13)] == EMPTY
                        || _board[i - (EXTENDED_SIDE + 13)] == EMPTY
                        || _board[i - ((EXTENDED_SIDE * 2) + 1)] == EMPTY
                        || _board[i - (EXTENDED_SIDE + EXTENDED_SIDE)] == EMPTY
                        || _board [i - (7 * 3)] == EMPTY
                        || _board[i + (7 + 2)] == EMPTY
                        || _board[i - 2] == EMPTY
                        || _board[i - 13] == EMPTY || _board[i + 13] == EMPTY
                        || _board[i + 2] == EMPTY
                        || _board[i - (7 + 2)] == EMPTY) {
                    return true;
                }
    ...
    void undo() {
        while (_undoSquares.peek() != null) {
            PieceColor savedResults = _undoPieces.pop();
            PieceColor currentPiece = get(_undoSquares.peek());
            int savedIndexes = _undoSquares.pop();
            incrPieces(savedResults, 1);
            incrPieces(currentPiece, -1);
            unrecordedSet(savedIndexes, savedResults);
        }

        _undoSquares.pop();
        _numMoves -= 1;
        _numJumps = _numberOfJumps.pop();
        _whoseMove = _whoseMove.opposite();
    ...
    boolean legalBlock(char c, char r) {
        if (_allMoves.isEmpty()) {
            return _board[index(c, r)].equals(EMPTY);
        } else {
            return false;
        }
    }
    ...
    void setBlock(char c, char r) {
        ...
        if (_board[index(c, r)] != BLOCKED) {
            if (!legalBlock(c, r)) {
                ...
                char symmetricCol = (char) (('a' + 'g') - c);
                char symmetricRow = (char) (('1' + '7') - r);
                unrecordedSet(c, r, BLOCKED);
                _totalOpen -= 1;
                if (_board[index(c, symmetricRow)] != BLOCKED) {
                    unrecordedSet(c, symmetricRow, BLOCKED);
                    _totalOpen -= 1;
                }
                if (_board[index(symmetricCol, r)] != BLOCKED) {
                    unrecordedSet(symmetricCol, symmetricRow, BLOCKED);
                    _totalOpen -= 1;
                }
                if (_board[index(symmetricCol, r)] != BLOCKED) {
                    unrecordedSet(symmetricCol, r, BLOCKED);
                    _totalOpen -= 1;
                }
            }
        }
    ...
    /** Stack of pieces formally at corresponding squares in _UNDOSQUARES. */
    private Stack<Integer> _numberOfJumps;
    /** Total number of blocks in the game. */
    private int _numBlocks;
    /** total number of moves and passes since the last clear
     *  or the creation of the board. */
