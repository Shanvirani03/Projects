...
class Model {
    ...
    void place(int k, int row, int col) {
        place(this._hand.get(k), row, col);
        this._hand.set(k, null);
        int numPieces = 0;
    }
    ...
    void clearFilledLines() {
        ...
        nrows = ncols = 0;
        int rowCells = counts[0].length;
        int colCells = counts[1].length;
        int numCells = 0;

        for (int r = 0; r < rowCells; r++) {
            if (counts[0][r] == this.width()) {
                nrows += 1;
                for (int i = 0; i < this.width(); i++) {
                    this._cells[r][i] = false;
                    numCells++;
                }
            }
        }
        for (int c = 0; c < colCells; c++) {
            if (counts[1][c] == this.height()) {
                ncols += 1;
                for (int j = 0; j < this.height(); j++) {
                    this._cells[j][c] = false;
                    numCells++;
                }
            }
        }
        ...
        int scoreCleared = scoreClearedLines(nrows, ncols);
        int cleared = (numCells - (nrows * ncols));
        _score += scoreCleared + cleared;
    }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int colCleared = this.height() * ncols;
        int rowCleared = this.width() * nrows;
        if (this._streakLength == 0) {
            return ((colCleared) + (rowCleared));
        }
        return ((colCleared) + (rowCleared)) * _streakLength;
    }
