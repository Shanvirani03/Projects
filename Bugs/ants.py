...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif self.can_contain(place.ant) is True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.is_hive is False:
            if place.bees and rnge >= self.min_range and rnge <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        amount_bees = list(self.place.bees)
        if self.health > amount:
            for bees in amount_bees:
                bees.reduce_health(amount)
        else: 
            for bees in amount_bees:
                bees.reduce_health(self.damage + amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        place_bee = self.place.bees
        if not choose_bee(place_bee):
            return
        elif self.chew_countdown > 0:
            self.chew_countdown -= 1
            return
        
        eat_bee = choose_bee(self.place.bees)
